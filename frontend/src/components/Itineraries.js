...
const Itineraries = ({
  ...
  var userFounded =
    userLogged && like.usersLiked.some((user) => user.userId === userLogged.id);
  ...
  const showMoreShowLess = (e) => {
    setToggleItineraries(
      toggleItineraries.button
  ...
  const likeToggle = async () => {
    if (userLogged) {
      ...
      const respuesta = await addOrRemoveLike(
        sendData,
        props.history,
        _id,
        userLS
  ...
  return (
    <div className="itineraryBanners">
      <div
        className="animate__animated animate__fadeIn itineraryBanner"
        style={{ backgroundImage: `url('/img/itineraryBackground.jpg')` }}
      >
        ...
        <div className="itineraryObservations">
          <svg
            onClick={!like.fetching ? () => likeToggle() : null}
            aria-hidden="true"
            focusable="true"
            data-prefix="far"
            data-icon="heart"
            className={userFounded ? "filled" : "empty"}
            role="img"
          ...
          >
            <path
              fill="currentColor"
              d={userFounded ? heartFilled : heartEmpty}
            ></path>
          </svg>
        ...
        <span className="likes">
          ...
          {userLogged && userFounded && like.usersLiked.length === 1
            ? `You like this!`
            : null}
          {like.usersLiked.length === 0 &&
            "Nobody like this yet, be the first!"}
        </span>
        <span
          style={{
            position: "relative",
            ...
            <div className="divActivities">
              {activity.activities.length !== 0 &&
                activity.activities.map((activity) => {
                  return (
                    <Activity key={activity._id} activityInfo={activity} />
                  );
                })}
            </div>
            ...
            <div className="commentaries">
              ...
              >
                {commentState.comments.length !== 0 ? (
                  ...
                  <div className="emptyCommentariesBox">
                    <h2 className="emptyCommentariesTitle">
              ...
              <div className="divInputComment">
                <input
                  ...
                  name="comment"
                  onKeyPress={(e) => enterToSend(e)}
                  placeholder="Write your comment here!"
                  onChange={(e) => readComment(e.target)}
                  type="text"
        ...
        <button
          ...
          onClick={(e) => showMoreShowLess(e.target.textContent)}
          onFocus={
            toggleItineraries.class === "hidden"
              ? () => loadActivities(_id)
              : null
          }
        >
          {toggleItineraries.text}
        </button>
      </div>
    </div>
  );
