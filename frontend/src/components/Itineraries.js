...
const Itineraries = ({userLogged, itinerary}) => {
  ...
  const [toggleLike, setToggleLike] = useState ({
    ...
    likesCount: itinerary.usersLiked.length,
    class:"empty"
  })
  ...
  const likeToggle = (async () => {
    var usersLiked = {userName: userLogged.id}
    const id = itinerary._id
    if (userLogged) {
      const userFounded = itinerary.usersLiked.filter(user => user.userName === userLogged.id)
      if (toggleLike.likesCount !== 0 && userFounded.length!==0){
        await axios.put(`http://localhost:4000/api/itinerary/removeLike/${id}`, {usersLiked})
      } else{     
        await axios.put(`http://localhost:4000/api/itinerary/addLike/${id}`, {usersLiked})
      } 
    setToggleLike(toggleLike.liked
      ? {liked: false, likedFill: heartEmpty, likesCount: itinerary.usersLiked.length -=1, class: "empty"}
      : {liked: true, likedFill: heartFilled, likesCount: itinerary.usersLiked.length +=1, class: "filled" })    
    } else{
